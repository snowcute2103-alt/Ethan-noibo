...
/**
 * SphereImageGrid — 3D avatar sphere, ảnh phân bố theo Fibonacci sphere,
 * hỗ trợ kéo xoay (momentum), tự xoay, và modal xem chi tiết.
 */
...
export interface ImageData {
  id: string;
  /** URL ảnh; để rỗng để hiện icon thay thế (chưa có ảnh đại diện). */
  src: string;
  alt: string;
  title?: string;
  description?: string;
        ...
        >
          <div className="relative h-full w-full overflow-hidden rounded-full border-2 border-white/20 shadow-lg">
            {image.src ? (
              ...
              <div className="flex h-full w-full items-center justify-center bg-[#FDEEF2] text-[#FF6F91]">
                <UserIcon className="h-1/2 w-1/2" strokeWidth={1.75} />
              </div>
            )}
          </div>
        </div>
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        ...
        >
          <div className="relative aspect-square bg-[#FDEEF2]">
            {selectedImage.src ? (
              <img src={selectedImage.src} alt={selectedImage.alt} className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full w-full items-center justify-center text-[#FF6F91]">
                <UserIcon className="h-1/4 w-1/4" strokeWidth={1.5} />
              </div>
            )}
            <button
              onClick={() => setSelectedImage(null)}
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6">
              {selectedImage.title && <h3 className="mb-1 text-xl font-bold text-navy">{selectedImage.title}</h3>}
              {selectedImage.description && <p className="text-muted">{selectedImage.description}</p>}
            </div>
          )}
        </div>
      </div>
